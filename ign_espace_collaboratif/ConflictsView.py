...
def resumeGeometry(obj):
    ...
    print("[INFO] géométrie : {}".format(geom))
    ...
    # 2) Dimension
    # --------------------------------------------
    # TODO reprendre la recherche de la dimension avec copilot
    # has_z = geom.is3D()
...
class ConflictsView(QtWidgets.QDialog, FORM_CLASS):
    ...
    def __zoomZoom(self, feature):
        ...
        self.__iface.mapCanvas().zoomScale(1500)  # zoom à 1:1500
        self.__iface.mapCanvas().refresh()

    @staticmethod
    def displayAllLayers():
    ...
    def __setAttributes(self, feature):
        ...
        for k, v in resultats.items():
            ...
            if k == 'geometrie':
                ...
                item = QtWidgets.QTableWidgetItem(resumeGeometry(geom))
            else:
    ...
    def __deleteObjectConflict(self):
        ...
        if len(layers) == 1:
            ...
            layers[0].deleteFeature(feature.id())
    ...
    def __createReport(self):
        ...
        # 2. Trouver la couche signalement et la passer en édition
        self.__editReportLayer()
    ...
    def __editReportLayer(self):
        layers = self.__project.mapLayersByName(cst.nom_Calque_Signalement)
        if len(layers) == 0:
            return
        # Rendre éditable
        if not layers[0].isEditable():
            layers[0].startEditing()
        # Sélection dans le panneau des couches
        root = self.__project.layerTreeRoot()
        node = root.findLayer(layers[0].id())
        if node:
            self.__iface.layerTreeView().setCurrentNode(node)
